...
import { getLocalizedContent, type MinimogSection } from "@/lib/content"
...
const defaultSections: MinimogSection[] = [
  { id: "minimog-header", type: "header", enabled: true, settings: { sticky_header: true } },
  {
    ...
    blocks: {
      "slide-0": {
        ...
        settings: {
          ...
          button_link: "/fr/minimog/collections",
  ...
  { id: "minimog-featured-collection", type: "featured-collection", enabled: true, settings: { title: "Beliebte Container" } },
  { id: "minimog-footer", type: "footer", enabled: true, settings: { show_social: true, newsletter: true } },
]
...
export default async function MinimogHome({
  ...
  try {
    ...
    const minimog = localized.minimog
    if (minimog?.sections) {
      const order = minimog.sectionOrder ?? minimog.sections.map((s) => s.id)
      const byId = new Map(minimog.sections.map((s) => [s.id, s]))
      sections = order.map((id) => byId.get(id)).filter((s): s is NonNullable<typeof s> => !!s) as typeof defaultSections
    }
  } catch {
    // keep default
  }
